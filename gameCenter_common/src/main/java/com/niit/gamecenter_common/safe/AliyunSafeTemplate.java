...
import com.niit.aliyuncs.green.extension.uploader.ClientUploader;
...
@Slf4j
public class AliyunSafeTemplate {

    private IAcsClient client;

    private IClientProfile profile;

    private AliyunProperties greenProperties;

    public AliyunSafeTemplate(AliyunProperties greenProperties) {
        this.greenProperties = greenProperties;
        try {
            profile = DefaultProfile
                    .getProfile("cn-shanghai", greenProperties.getAccessKey(), greenProperties.getSecret());
            DefaultProfile
...
//                    设置默认错误返回内容
    ...
    /**
     *
     * @param imageBytes 图片的字节数组对象
     * @return
     */
    public Map<String,String> imageScan(byte[] imageBytes){
        ImageSyncScanRequest imageSyncScanRequest = new ImageSyncScanRequest();
        // 指定API返回格式。
        imageSyncScanRequest.setAcceptFormat(FormatType.JSON);
        // 指定请求方法。
